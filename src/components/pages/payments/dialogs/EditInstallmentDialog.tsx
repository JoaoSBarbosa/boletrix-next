...
export const EditInstallmentDialog = ({installment, iconSize = 24, reload}: EditInstallmentDialogProps) => {
    ...
    async function handleUpdateRegister() {
        ...
        console.log("Nº: ", installmentNumber)
        try {

            const data: AxiosResponse<InstallmentResponseType> = await ApiConnection(window.location.href).patch(`/installments/${installment?.id}`, {
                amount,
                paymentDate,
                paymentTime,
                installmentNumber,
    ...
    return (
        <Modal
            ...
            trigger={
                ...
                >
                    <NotePencilIcon size={iconSize} weight={'regular'} color={"#075985"}
        ...
        >
            {showLoading ?
                ...
                <Loading title={"Atualizando registro"} message={"Por favor, aguarde. Estamos realizando a operação"}/>
                :
                <Form.FormColumns
                    justifyContent={'flex-start'} classNameCustom={"p-2 my-2"}>
