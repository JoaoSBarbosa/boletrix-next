...
export function InputFile({
  ...
  const handleListFilesSelected = (event: any) => {
    ...
    for (const file of event.target.files) filesNameList.push(file.name);
    ...
    >
      ...
      >
        ...
        <div className={"flex flex-row "}>
          ...
          />
          <div className={styles.inputIcon}>
            <FileDashed size={"50%"} weight="bold" />
          </div>
        </div>
      </div>

      {filesNameList.map((fileName) => (
        <div className={styles.fileItem} aria-label={"Item " + fileName}>
          <div className={styles.fileItemIcon}>
            <FilePlus size={"80%"} weight="fill" />
          </div>
...
interface InputTextAndButtonProps
...
export function InputTextAndButton({
  ...
  return (
    <Label.Root
      className={`w-full p-1 flex items-start justify-center flex-col border-b-2 border-gray-300 font-light text-[15px] text-[#898f9a] ${
        isDisabled ? "bg-gray-100" : ""
      }`}
      style={{
        transition: "0.5s",
    ...
    >
      ...
      <div
        className={`w-full h-full flex items-center justify-between flex-row pt-2 `}
      >
        <input
          id={title.toLowerCase().replace(/ /g, "")}
        ...
        {children && (
          <div
            className={`w-1/4 h-full flex items-center justify-center flex-col`}
          >
            {children}
          </div>
        )}
...
export function InputSelect({
  ...
  return (
    <label
      className={`flex  items-start justify-center flex-col font-light text-[15px] text-[#898f9a] ${
        isDisabled ? "bg-gray-100" : ""
      }`}
      style={{
        transition: "0.5s",
    ...
    >
      ...
      <div
        className={`w-full h-full flex items-center justify-between flex-row`}
      >
        <select
          id={title.toLowerCase().replace(/ /g, "")}
...
const InputTextRef = React.forwardRef<HTMLInputElement, InputTextPropsRef>(
  (
    ...
    if (compactMode) {
      return (
        <Label.Root
          className={`${styles.compactLabelRoot} ${
            isDisabled ? "bg-gray-100" : ""
          } ${cursorText ? "cursor-text" : ""}`}
          style={{
            transition: "0.5s",
        ...
        >
          <div className={"flex gap-1 items-center"}>
            ...
            {isBadge && badgeText ? (
              <span
                className={`${badgeColor} text-xs font-medium px-1 py-0.5 rounded`}
              >
                {badgeText}
              </span>
            ) : null}
          ...
          <div className={styles.inputTextContainer}>
            ...
            {children && (
              <div className={styles.compactInputIcon}>{children}</div>
            )}
          </div>
        </Label.Root>
      );
